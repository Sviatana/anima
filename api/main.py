...
def reply_motiv(style, humor)->str:
    return (
        "Возвращаем мотивацию: Зачем-слой → Порог 15 минут → Убрать трение → Ритуал старта.\n"
...
def reply_sad(style, humor)->str:
    return (
        "С грустью бережно: назвать чувство (0–10) → 10 минут «побыть» → поддержать тело → один контакт с миром.\n"
        "Я рядом. Что было бы самым бережным сейчас?"
    )

def reply_mindful(style, humor)->str:
    return (
        "Осознанность 2 минуты: скан тела (ступни→лицо), 10 спокойных выдохов, считай только выдохи.\n"
...
def reply_smart(style, humor)->str:
    return (
        "Оформим цель по SMART: «До [дата] я [результат]; измерю по [метрика]; важно потому что [значимость]».\n"
        "Кинь черновик — отточим."
    )

def reply_eisen(style, humor)->str:
    return (
        "Матрица Эйзенхауэра: I — сегодня; II — план; III — делегирую; IV — убираю.\n"
...
def reply_negotiation(style, humor)->str:
    return (
        "Переговоры (интересы → варианты → критерии). Скрипт: «Хочу договориться так, чтобы обеим сторонам было хорошо. Что для вас самое важное?»\n"
        "Опиши кейс — соберём план."
    )

def reply_interview(style, humor)->str:
    return (
        "Собеседование: 3 истории по STAR, питч 60–90 сек, вопросы к компании и письмо-резюме после.\n"
        "Набросаем 1 историю?"
    )

def reply_weekly(style, humor)->str:
    return (
        "Еженедельный обзор: инбокс-ноль → 3 сделанных/3 урока/1 радость → 3 приоритета недели → бронь в календаре → один ритуал заботы.\n"
        "Нужен чек-лист?"
    )

def reply_study(style, humor)->str:
    return (
        "Учёба: помидоры 25/5, карта тем, метод Фейнмана, интервальные повторы (сегодня/завтра/3 дня/неделя).\n"
        "Какая тема сейчас?"
    )

def reply_adhd(style, humor)->str:
    return (
        "АДХД-дружественный режим: таймер, визуальный список, тёплый старт 5 минут, правило 80%, быстрые награды.\n"
...
def reply_parent(style, humor)->str:
    return (
        "Поддерживающее родительство: заметить хорошее (факт), выбор из двух хороших, валидировать эмоцию — потом границы и план.\n"
...
# для быстрого выбора по коду
CODE2FN: Dict[str, IntentFn] = {
    "decision": reply_decision,
    "stress": reply_stress,
    "procrastination": reply_procras,
    "goals": reply_goals,
    "boundaries": reply_boundaries,
    "relations": reply_relation,
    "imposter": reply_imposter,
    "burnout": reply_burnout,
    "sleep": reply_sleep,
    "motivation": reply_motiv,
    "anger": reply_anger,
    "sadness": reply_sad,
    "mindfulness": reply_mindful,
    "cbt": reply_cbt,
    "smart": reply_smart,
    "eisenhower": reply_eisen,
    "pomodoro": reply_pomodoro,
    "finance_anxiety": reply_finance,
    "partner_search": reply_partner,
    "career": reply_career,
    "public_speaking": reply_speaking,
    "negotiations": reply_negotiation,
    "interview": reply_interview,
    "weekly_review": reply_weekly,
    "study": reply_study,
    "adhd_mode": reply_adhd,
    "declutter": reply_declutter,
    "parenting": reply_parent,
    "healthy_habits": reply_habits,
    "creative_block": reply_creative,
    "relocation": reply_reloc,
    "gratitude": reply_grat,
    "morning_routine": reply_morning,
}
...
def compose_menu(uid:int)->str:
    # сохраним карту номеров → код
    mapping = {str(i+1): code for i,(code,_) in enumerate(MENU_LIST[:10])}
    set_state(uid, {"menu_map": mapping})
    lines = ["Выбери тему цифрой, а я сразу предложу план:\n"]
    for i,(code,title) in enumerate(MENU_LIST[:10], start=1):
        lines.append(f"{i}) {title}")
    lines.append("\nМожно написать свою тему словами — я пойму.")
    return "\n".join(lines)

def try_menu_choice(uid:int, text:str, style:Dict[str,str], humor_on:bool)->Optional[str]:
    st = app_state(uid)
    ...
    if t in mapping:
        ...
        fn = CODE2FN.get(code)
        if fn:
            # сбросить меню после выбора
            set_state(uid, {"menu_map": {}})
...
def build_reply(uid:int, user_text:str, humor_on:bool)->str:
    ...
    # слишком коротко/расплывчато — покажем меню
    if len(t) < 4:
        return compose_menu(uid)
...
async def webhook(update: TelegramUpdate, request: Request):
    ...
    if text.lower() in ("/start","start"):
        set_state(uid, {"intro_done":False, "name":None, "kno_idx":None, "kno_done":False, "menu_map": {}})
    ...
    # выбор пункта меню по цифре
    pr = q("SELECT ei,sn,tf,jp FROM psycho_profile WHERE user_id=%s",(uid,))
    p = pr[0] if pr else {"ei":0.5,"sn":0.5,"tf":0.5,"jp":0.5}
    style = comms_style(p)
    menu_choice = try_menu_choice(uid, text, style, humor_on)
    if menu_choice:
        draft = menu_choice
    else:
    ...
    if quality_score(text, draft) < 0.55:
        # если реплика всё ещё кажется расплывчатой — предложим меню
        draft = compose_menu(uid)
